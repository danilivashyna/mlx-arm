...
bool VulkanContext::initialize(bool enable_validation) {
...
#ifdef __ANDROID__
    // Disable validation layers on Android by default (not available in release builds)
    enable_validation = false;
    LOGI("   Android detected - validation layers disabled");
#endif
    ...
    LOGI("✅ Vulkan available, creating instance...");
...
bool VulkanContext::create_instance(bool enable_validation) {
    ...
    LOGI("   Step 1/3: Creating VkInstance...");
    LOGI("      Extensions: %d", create_info.enabledExtensionCount);
    for (uint32_t i = 0; i < create_info.enabledExtensionCount; i++) {
        LOGI("        - %s", create_info.ppEnabledExtensionNames[i]);
    }
    LOGI("      Layers: %d", create_info.enabledLayerCount);
    for (uint32_t i = 0; i < create_info.enabledLayerCount; i++) {
        LOGI("        - %s", create_info.ppEnabledLayerNames[i]);
    }
    
    VkResult result = vkCreateInstance(&create_info, nullptr, &instance_);
    if (result != VK_SUCCESS) {
        LOGE("      ❌ vkCreateInstance FAILED with error code: %d", result);
        LOGE("         VK_SUCCESS=0, VK_ERROR_OUT_OF_HOST_MEMORY=-1");
        LOGE("         VK_ERROR_OUT_OF_DEVICE_MEMORY=-2");
        LOGE("         VK_ERROR_INITIALIZATION_FAILED=-3");
        LOGE("         VK_ERROR_LAYER_NOT_PRESENT=-7");
        LOGE("         VK_ERROR_EXTENSION_NOT_PRESENT=-8");
        LOGE("         VK_ERROR_INCOMPATIBLE_DRIVER=-9");
        return false;
    }
    
    LOGI("      ✅ VkInstance created successfully!");
...
std::vector<VkPhysicalDevice> VulkanContext::get_physical_devices() const {
    LOGI("   Step 2/3: Enumerating physical devices...");
    ...
    LOGI("      Found %u physical device(s)", device_count);
    ...
    for (uint32_t i = 0; i < device_count; i++) {
        ...
        LOGI("      Device %u: %s", i, props.deviceName);
        LOGI("         Type: %d", props.deviceType);
        LOGI("         API: %u.%u.%u", 
